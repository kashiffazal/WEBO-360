<?php
  ...
  $apiSpamCount = sizeof($bs['spam']);
  if($apiBounceCount > 0 || $apiSpamCount > 0){#if there is bounce email then update
    #If DB bounce count and Api bounce count are Equal the don't update because it's already updated in DB
    if(trim($c_data['bounceCount']) != $apiBounceCount || trim($c_data['spamCount']) != $apiSpamCount){

      if(trim($c_data['bounceCount']) != $apiBounceCount){
        $statusId = '3';
        $colName = 'bounce_by_campaign';
      }//End if condition
      
      if(trim($c_data['spamCount']) != $apiSpamCount){
        $statusId = '4';
        $colName = 'spam_by_campaign';
      }//End if condition

      $sql_query = "UPDATE $subscribers_table SET status = '$statusId', $colName = '$campaign_id' WHERE (";
